...
import maskFixture from './middlewares/proxy/maskFixture';
...
export default (fixtureDirs: string[] = [], realApiBaseUrl?: string, outputDir?: string) => {
	...
	app.use(
		morgan(
			`${chalk.magenta('[Stub server]')} ${chalk.green(':method')} :url ${chalk.magenta(':status')} ${chalk.cyan(
				':response-time ms',
			)} HTTP/:http-version :date[iso]`,
		),
	);

	console.log(`${chalk.magenta('[zStub server]')} looking for fixtures in:`);
	console.log(chalk.yellow(fixtureDirs.join(',')));
	console.log(`${chalk.magenta('[Stub server]')} found fixtures:`);
	console.log(
		extractEndpoints(listAllFixtures(fixtureDirs))
			.map((e) => `${chalk.green(e.method)} ${e.endpoint} { ${chalk.yellow(Object.keys(e.variants).join(', '))} }`)
			.join('\n'),
	);
	...
	app.get(
		...
		(req: Request, res: Response, next: NextFunction): void => {
			...
			if (req.query.set) variants = req.query.set.split(',');
			else if (req.query.add) variants = cleanupVariantsConflicts(variants.concat(req.query.add.split(',')));
			else if (req.query.clear) variants = [];
			res.cookie('variants', variants.join(','), { encode: String } as express.CookieOptions).json({
				variants,
				possibleVariants: extractEndpoints(listAllFixtures(fixtureDirs)).reduce(
					(acc: string[], endpoint: Endpoint) =>
						acc.concat(
	...
	app.use(
		(req: Request, res: Response, next: NextFunction): void => {
			...
			// Fixtures to mask
			const maskedFixtures = require('../maskedFixtures.json');
			...
			if (foundEndpoint && foundFixturePath) {
				...
				typeof fixture === 'function'
					? fixture(req, res, next)
					: res.json(maskFixture(foundEndpoint.endpoint, fixture, maskedFixtures));
			} else {
				next();
			}
		},
