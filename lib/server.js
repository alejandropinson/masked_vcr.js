...
var maskFixture_1 = require("./middlewares/proxy/maskFixture");
...
exports.default = function (fixtureDirs, realApiBaseUrl, outputDir) {
    ...
    console.log(endpoints_1.extractEndpoints(listAllFixtures_1.default(fixtureDirs))
        .map(function (e) { return chalk.green(e.method) + " " + e.endpoint + " { " + chalk.yellow(Object.keys(e.variants).join(', ')) + " }"; })
        .join('\n'));
    if (realApiBaseUrl) {
    ...
    app.get('/variants', function (req, res, next) {
        ...
        if (req.query.set)
            ...
            variants = [];
        res.cookie('variants', variants.join(','), { encode: String }).json({
            variants: variants,
    ...
    app.use(function (req, res, next) {
        ...
        // Fixtures to mask
        var maskedFixtures = require('../maskedFixtures.json');
        if (foundEndpoint && foundFixturePath) {
            ...
            typeof fixture === 'function'
                ...
                : res.json(maskFixture_1.default(foundEndpoint.endpoint, fixture, maskedFixtures));
        }
        else {
            next();
        }
